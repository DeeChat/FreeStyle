...
def tokenizer(string):
	...
	for i in range(len(string)):
        if string[i] >= u'\u4e00' and string[i] <= u'\u9fa5':
            char = char + string[i]
	regx_space = re.compile('\s',re.X)
	char = re.sub(regx_space,'',char)
	seg_list = jieba.cut(char, cut_all = False)
...
def main():
	...
	regx=re.compile('[a-zA-Z0-9:]',re.X)
	for input in inputs:
		...
		for file in files:
			with codecs.open(input_path + '/' + file,'r','utf-8') as fopen:
				...
				if re.search(regx,string_lyric) is None:
					raw_lyric = [ clean_lyric(x) for x in raw_lyric ]
